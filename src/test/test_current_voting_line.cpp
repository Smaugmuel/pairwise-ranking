...
void counterLengthEqualsTotalLength() {
	ASSERT_EQ(
		VotingRound::create({ "i1", "i2", "i3", "i4" }, false).value().currentVotingLine().value(),
		std::string{ "(1/6) H: help. A: \'i1\'. B: \'i2\'. Your choice: " });
}
void counterLengthIsLessThanTotalLength() {
	ASSERT_EQ(
		VotingRound::create({ "i1", "i2", "i3", "i4", "i5" }, false).value().currentVotingLine().value(),
		std::string{ "( 1/10) H: help. A: \'i1\'. B: \'i2\'. Your choice: " });
	ASSERT_EQ(
		VotingRound::create({
			"it1", "it2", "it3", "it4", "it5",
			"it6", "it7", "it8", "it9", "i10",
			"i11", "i12", "i13", "i14", "i15", }, false).value().currentVotingLine().value(),
			std::string{ "(  1/105) H: help. A: \'it1\'. B: \'it2\'. Your choice: " });
}
void itemIsShorterThanLongestItem() {
	ASSERT_EQ(
		VotingRound::create({
			"item1", "shortItem2", "longestItem3" }, false).value().currentVotingLine().value(),
		std::string{ "(1/3) H: help. A: \'item1\'.        B: \'shortItem2\'.   Your choice: " });
	ASSERT_EQ(
		VotingRound::create({
			"longestItem1", "i2", "item3" }, false).value().currentVotingLine().value(),
		std::string{ "(1/3) H: help. A: \'longestItem1\'. B: \'i2\'.           Your choice: " });
}
void itemLengthIsEqualToHeaderLength() {
	ASSERT_EQ(
		VotingRound::create({
			"a", "b", "c" }, false).value().currentVotingLine().value(),
			std::string{ "(1/3) H: help. A: \'a\'. B: \'b\'. Your choice: " });
	ASSERT_EQ(
		VotingRound::create({
			"i1", "i2", "i3" }, false).value().currentVotingLine().value(),
			std::string{ "(1/3) H: help. A: \'i1\'. B: \'i2\'. Your choice: " });
	ASSERT_EQ(
		VotingRound::create({
			"item1", "item2", "item3" }, false).value().currentVotingLine().value(),
